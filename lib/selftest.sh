...
selftest_safe() {
    ...
    _selftest_run "防火墙备份" "临时文件" "\"$TT_HOME/tiantian.sh\" firewall backup >/dev/null"
    ...
    _selftest_run "Docker 清理预案" "只读" "\"$TT_HOME/tiantian.sh\" docker prune-plan safe >/dev/null || true"
    _selftest_run "SSH 加固预案" "只读" "\"$TT_HOME/tiantian.sh\" ssh harden-plan 22 >/dev/null || true"
    ...
    _selftest_run "系统基础设置状态" "只读" "\"$TT_HOME/tiantian.sh\" system status >/dev/null || true"
    _selftest_run "系统时区预案" "只读" "\"$TT_HOME/tiantian.sh\" system timezone-plan Asia/Shanghai >/dev/null || true"
    _selftest_run "系统主机名预案" "只读" "\"$TT_HOME/tiantian.sh\" system hostname-plan tt-selftest >/dev/null || true"
    _selftest_run "地址族优先级预案" "只读" "\"$TT_HOME/tiantian.sh\" system ip-prefer-plan ipv4 >/dev/null || true"
    _selftest_run "历史文件概览" "只读" "\"$TT_HOME/tiantian.sh\" history files >/dev/null || true"
    _selftest_run "历史记录查看" "只读" "\"$TT_HOME/tiantian.sh\" history list 5 >/dev/null || true"
    _selftest_run "历史清空预案" "只读" "\"$TT_HOME/tiantian.sh\" history clear-plan >/dev/null || true"
    _selftest_run "系统日志概览" "只读" "\"$TT_HOME/tiantian.sh\" syslog overview >/dev/null || true"
    _selftest_run "最近系统日志" "只读" "\"$TT_HOME/tiantian.sh\" syslog recent 5 >/dev/null || true"
    _selftest_run "认证日志查看" "只读" "\"$TT_HOME/tiantian.sh\" syslog auth 5 >/dev/null || true"
    _selftest_run "journal 清理预案" "只读" "\"$TT_HOME/tiantian.sh\" syslog vacuum-plan 7d >/dev/null || true"
    ...
    _selftest_run "同步任务计划列表" "只读" "\"$TT_HOME/tiantian.sh\" tasks crontab >/dev/null || true"
    _selftest_run "安全工具状态" "只读" "\"$TT_HOME/tiantian.sh\" security status >/dev/null || true"
    _selftest_run "Fail2ban 预案" "只读" "\"$TT_HOME/tiantian.sh\" security fail2ban-plan >/dev/null"
    _selftest_run "ClamAV 扫描预案" "只读" "\"$TT_HOME/tiantian.sh\" security clamav-plan /tmp >/dev/null"
    _selftest_run "磁盘分区总览" "只读" "\"$TT_HOME/tiantian.sh\" disk overview >/dev/null || true"
    _selftest_run "磁盘挂载状态" "只读" "\"$TT_HOME/tiantian.sh\" disk mounts >/dev/null || true"
    _selftest_run "未挂载候选分区" "只读" "\"$TT_HOME/tiantian.sh\" disk candidates >/dev/null || true"
    _selftest_run "磁盘健康状态" "只读" "\"$TT_HOME/tiantian.sh\" disk health >/dev/null || true"
